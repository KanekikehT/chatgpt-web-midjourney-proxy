...
async function subV2(b: { k: string; n: string }) {
  if (chat.value.opt?.buttons === undefined)
    return
  const i = getIndex(chat.value.opt?.buttons, b)
  if (await checkPoints()) { // 检查是否有足够的积分
    const deductionResult = await deductPoints() // 尝试扣除积分
    if (deductionResult === '积分更新成功。') {
      ...
      homeStore.setMyData({ act: 'draw', actData: obj })
      ms.success('操作成功，积分已扣除。')
    }
    else {
      ms.error(deductionResult) // 显示错误信息
    }
  }
  else {
